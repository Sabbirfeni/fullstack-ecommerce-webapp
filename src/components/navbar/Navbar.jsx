...
export default function Navbar() {
  ...
  const { mode, mobileMenuOpen, setMobileMenuOpen, setSearchKey } = context;
    ...
    <div className="fixed w-full z-50 border-b border-[#ececec] bg-[#f8f8f8]">
      <div className="flex container mx-auto md:py-2 py-2 sm:px-5 px-4">
        <div className="navbar-left flex  items-center justify-between md:space-x-6 space-x-3 flex-1 lg:flex-none">
          ...
          <div>
            <Link to="/">
              <img
                src={logo}
                className="lg:w-15 md:w-14 w-12"
                alt="website logo"
              />
            </Link>
          </div>
          ...
          <div className="relative">
            ...
            <svg
              onClick={(e) => getSearchItems(e)}
              className="absolute sm:top-3 top-2.5 right-3 w-4 cursor-pointer"
              fill="#000000"
          ...
          <div className="flex lg:hidden gap-3 md:gap-5">
            <Link to="/cart" className="relative">
              {cartItems.length > 0 && (
                <div className="absolute -top-3 -right-3 w-5 h-5 flex items-center justify-center text-xs rounded-full text-white p-2 bg-[#3fd1eb]">
                  {cartItems.length}
                </div>
              )}
        ...
        {profileMenuOpen && (
          <div
            onClick={handleProfileMenu}
            className="w-[100%] h-screen absolute top-0 bottom-0 right-0 z-40 bg-[#00000000]"
          ></div>
        )}
      </div>
    </div>
