...
function Overview() {
  ...
  const { mode, allProducts, deleteProduct, editHandle, orders, users } =
    context;
  const activerOrderNum = orders.filter(
    (item) => item.orderStatus !== "completed"
  ).length;
  const completedOrderNum = orders.filter(
    (item) => item.orderStatus === "completed"
  ).length;
  const totalOrders = [];
  orders.map((order) => {
      ...
      <div className="overview-cards-container flex md:flex-row flex-col md:gap-3 gap-2">
        ...
        <div className="overview-card bg-[#fff] flex flex-col flex-1 p-4 space-y-12 border rounded-md">
          ...
          <div className="flex items-center justify-between">
            ...
            <p className="text-sm text-[#2ec467]">
              completed: {completedOrderNum}
            </p>
          </div>
        </div>
      </div>
              ...
              {allProducts.slice(0, 6).map((item, index) => {
                ...
                return (
                  <tr key={`${time}`} className="bg-white border-b ">
                    ...
                    <td className="px-6 py-4">{catergory}</td>
